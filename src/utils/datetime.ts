...
export function getRelativeTopInDay(
  ...
  return (
    (100 * ((date.hour() - minHour) * 60 + date.minute())) / totalMinutesInRange
  );
}
...
export function enrichEvents<T extends ICalendarEventBase>(
  ...
  const eventsWithOverlaps = baseEvents.map((event, index) => {
    ...
    if (event.start < groupEndTime) {
      ...
      if (index === baseEvents.length - 1) {
        overlapCountingPointers.push(
          ...Array(overlapCounting).fill(overlapCounting)
        );
      }
  ...
  eventsWithOverlaps.forEach((event, index) => {
    ...
    if (startDate === endDate) {
      ...
      const amountOfDaysBetweenDates = dayjs(enrichedEvent.start).diff(
        enrichedEvent.end,
        "day"
      );
...
export function getEventSpanningInfo(
  ...
  const eventDuration =
    Math.floor(
      dayjs
        .duration(dayjs(event.end).endOf("day").diff(dayjs(event.start)))
        .asDays()
    ) + 1;
  const eventDaysLeft =
    Math.floor(
      dayjs.duration(dayjs(event.end).endOf("day").diff(date)).asDays()
    ) + 1;
